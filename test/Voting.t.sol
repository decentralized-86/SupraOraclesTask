...
import {DecentralizedVoting} from "../contracts/Voting.sol";

/**
 * @title DecentralizedVotingTest
 * @dev Contract for testing the DecentralizedVoting contract. Includes setup and various test cases.
 */
contract DecentralizedVotingTest is Test {
    DecentralizedVoting public votingContract; 
    ...
    function setUp() public {
        owner = address(this); 
        votingContract = new DecentralizedVoting(1);
        ...
        votingContract.registerCandidate("Alice");
        votingContract.registerCandidate("Bob");
    }
	...
	function testDeclareWinner() public {
    	...
    	vm.prank(owner);
    	uint winningCandidateId = votingContract.declareWinner();
    	console.log("winningCandidateId", winningCandidateId);

    	// Fetch and log the declared winner's details
    	(uint id, string memory name, uint voteCount) = getCandidateDetails(winningCandidateId);
 	...
 	*/
	function getCandidateDetails(uint candidateId) internal view returns (uint, string memory, uint) {
